...
//!
//! Tests to ensure that Slint properties are correctly synchronized with Rust state
...
mod ui_sync_tests {
    ...
    async fn test_profile_defaults_applied_on_startup() {
        ...
        controller.handle_profile_change("Gaming")
            .expect("Failed to set Gaming profile");

        // Get initial state
        let initial_state = controller.get_state().expect("Failed to get initial state");
        eprintln!("[TEST] Initial state: profile={}, Polly={}, MGLRU={}",
            initial_state.selected_profile,
            initial_state.use_polly,
            initial_state.use_mglru
        );

        // Apply current profile defaults (simulates startup behavior)
        controller.apply_current_profile_defaults()
            .expect("Failed to apply profile defaults");

        // Get updated state
        let updated_state = controller.get_state().expect("Failed to get updated state");
        eprintln!("[TEST] Updated state: profile={}, Polly={}, MGLRU={}",
            updated_state.selected_profile,
            updated_state.use_polly,
            updated_state.use_mglru
        );

        // Verify the profile's defaults are applied
        assert!(!updated_state.selected_profile.is_empty(), "Profile should not be empty");
        
        // Gaming profile should have: Polly=true, MGLRU=true
        if updated_state.selected_profile == "Gaming" {
            assert!(updated_state.use_polly, "Gaming profile should have Polly enabled");
            assert!(updated_state.use_mglru, "Gaming profile should have MGLRU enabled");
        }
    }
    ...
    async fn test_profile_change_updates_all_options() {
        ...
        eprintln!("[TEST] Gaming profile state: Polly={}, MGLRU={}",
            gaming_state.use_polly,
            gaming_state.use_mglru
        );
        ...
        controller.handle_profile_change("Server")
            .expect("Failed to change to Server profile");

        let server_state = controller.get_state().expect("Failed to get Server state");
        eprintln!("[TEST] Server profile state: Polly={}, MGLRU={}",
            server_state.use_polly,
            server_state.use_mglru
        );
    ...
    async fn test_individual_option_changes() {
        ...
        controller.handle_polly_change(false)
            .expect("Failed to toggle Polly");

        let after = controller.get_state().expect("Failed to get state after Polly toggle");
        eprintln!("[TEST] After Polly toggle: Polly={}", after.use_polly);
        
        assert!(!after.use_polly, "Polly should be disabled after manual toggle");
    ...
    async fn test_mglru_option_changes_independently() {
        ...
        controller.handle_mglru_change(false)
            .expect("Failed to toggle MGLRU");

        let after = controller.get_state().expect("Failed to get state after MGLRU toggle");
        eprintln!("[TEST] After MGLRU toggle: MGLRU={}", after.use_mglru);
        
        assert!(!after.use_mglru, "MGLRU should be disabled after manual toggle");
    ...
    async fn test_state_persistence_across_profile_changes() {
        ...
        let gaming_state = controller.get_state().expect("Failed to get Gaming state");
        eprintln!("[TEST] Gaming profile: Polly={}, MGLRU={}",
            gaming_state.use_polly,
            gaming_state.use_mglru
        );

        // Change to Workstation
        controller.handle_profile_change("Workstation")
            .expect("Failed to set Workstation profile");

        let workstation_state = controller.get_state().expect("Failed to get Workstation state");
        eprintln!("[TEST] Workstation profile: Polly={}, MGLRU={}",
            workstation_state.use_polly,
            workstation_state.use_mglru
        );
        ...
        let gaming_state_2 = controller.get_state().expect("Failed to get Gaming state again");
        eprintln!("[TEST] Gaming profile again: Polly={}, MGLRU={}",
            gaming_state_2.use_polly,
            gaming_state_2.use_mglru
        );

        // Gaming defaults should be reapplied
        assert!(gaming_state_2.use_polly, "Gaming profile should have Polly enabled on reapply");
        assert!(gaming_state_2.use_mglru, "Gaming profile should have MGLRU enabled on reapply");
    }
    ...
    async fn test_whitelist_auto_discovery_synchronization() {
        ...
        let gaming_state = controller.get_state().expect("Failed to get Gaming state");
        eprintln!("[TEST] Gaming profile: use_modprobed={}, use_whitelist={}",
            gaming_state.use_modprobed,
            gaming_state.use_whitelist
        );

        // Both should be synchronized with profile setting
        assert_eq!(gaming_state.use_modprobed, gaming_state.use_whitelist,
                   "Whitelist must be synchronized with modprobed on profile change");
        assert!(gaming_state.use_modprobed, "Gaming profile should enable modprobed");
        assert!(gaming_state.use_whitelist, "Whitelist should be enabled when modprobed is enabled");
    }
    ...
    async fn test_modprobed_override_disables_whitelist() {
        ...
        controller.handle_modprobed_change(false)
            .expect("Failed to toggle modprobed");

        let after = controller.get_state().expect("Failed to get state after modprobed toggle");
        eprintln!("[TEST] After modprobed toggle: use_modprobed={}, use_whitelist={}",
            after.use_modprobed,
            after.use_whitelist
        );

        assert!(!after.use_modprobed, "modprobed should be disabled");
        assert!(!after.use_whitelist, "Whitelist must follow modprobed state");
    }
    ...
    async fn test_modprobed_enable_enables_whitelist() {
        ...
        controller.handle_modprobed_change(true)
            .expect("Failed to enable modprobed");

        let after = controller.get_state().expect("Failed to get state after modprobed re-enable");
        eprintln!("[TEST] After modprobed re-enable: use_modprobed={}, use_whitelist={}",
            after.use_modprobed,
            after.use_whitelist
        );

        assert!(after.use_modprobed, "modprobed should be enabled");
        assert!(after.use_whitelist, "Whitelist should be re-enabled with modprobed");
    }
    ...
    async fn test_lto_optional_with_profile_defaults() {
        ...
        let gaming_state = controller.get_state().expect("Failed to get Gaming state");
        eprintln!("[TEST] Step 1 - Gaming profile: selected_lto={}", gaming_state.selected_lto);

        // Gaming should have Thin LTO (profile default)
        assert_eq!(gaming_state.selected_lto, "thin", "Gaming profile default should be Thin LTO");

        // Step 2: User manually overrides to Full LTO
        controller.handle_lto_change("full")
            .expect("Failed to set LTO to Full");

        let override_state = controller.get_state().expect("Failed to get state after LTO override");
        eprintln!("[TEST] Step 2 - LTO override: selected_lto={}", override_state.selected_lto);

        assert_eq!(override_state.selected_lto, "full", "LTO should be set to full after user override");
        ...
        let server_state = controller.get_state().expect("Failed to get Server state");
        eprintln!("[TEST] Step 3 - Server profile: selected_lto={}", server_state.selected_lto);

        // Server profile should reset LTO to its default (Full)
        assert_eq!(server_state.selected_lto, "full", "Server profile default should be Full LTO");

        // Step 4: User overrides Server's default to Thin
        controller.handle_lto_change("thin")
            .expect("Failed to set LTO to Thin");

        let final_state = controller.get_state().expect("Failed to get final state");
        eprintln!("[TEST] Step 4 - Final LTO: selected_lto={}", final_state.selected_lto);

        assert_eq!(final_state.selected_lto, "thin", "LTO should be thin after user override for Server");
    }
    ...
    async fn test_lto_override_independence() {
        ...
        let after = controller.get_state().expect("Failed to get state after other changes");
        eprintln!("[TEST] After Polly changes: selected_lto={}, use_polly={}",
            after.selected_lto,
            after.use_polly
        );

        // LTO override should persist
        assert_eq!(after.selected_lto, "full", "LTO override should persist across other option changes");
    ...
    async fn test_gaming_profile_startup_sync_all_options() {
        ...
        eprintln!("[TEST] STEP 3: State at startup: profile={}, Polly={}, MGLRU={}",
            startup_state.selected_profile,
        ...
        assert_eq!(startup_state.selected_profile, "Gaming",
                   "Profile should be Gaming at startup");
        
        assert!(startup_state.use_polly,
                "CRITICAL: Gaming profile MUST have use_polly=true at startup");
        
        assert!(startup_state.use_mglru,
    ...
    async fn test_profile_startup_sync_all_variants() {
        ...
        for profile_name in profiles {
            ...
            controller.handle_profile_change(profile_name)
                .expect(&format!("Failed to set {} profile", profile_name));
            
            controller.apply_current_profile_defaults()
                .expect(&format!("Failed to apply defaults for {} profile", profile_name));
            
            let state = controller.get_state().expect("Failed to get state");
            
            eprintln!("[TEST] {} startup state: Polly={}, MGLRU={}, LTO={}",
                profile_name,
                state.use_polly,
                state.use_mglru,
                state.selected_lto
            ...
            match profile_name {
                ...
                "Workstation" => {
                    ...
                    assert_eq!(state.selected_lto, "thin", "Workstation should use Thin LTO");
                }
                "Server" => {
    ...
    async fn test_phase8_final_verification_gate_ui_state_sync() {
        ...
        let state_after_profile = controller.get_state()
            .expect("Failed to get state after profile change");
        
        eprintln!("[TEST] STEP 1 - State after Gaming profile:");
        eprintln!("[TEST]   selected_profile: {}", state_after_profile.selected_profile);
        eprintln!("[TEST]   use_polly: {}", state_after_profile.use_polly);
        eprintln!("[TEST]   use_mglru: {}", state_after_profile.use_mglru);
        eprintln!("[TEST]   selected_lto: {}", state_after_profile.selected_lto);
        eprintln!("[TEST]   use_modprobed: {}", state_after_profile.use_modprobed);
        eprintln!("[TEST]   use_whitelist: {}", state_after_profile.use_whitelist);
        ...
        assert_eq!(state_after_profile.selected_lto, "thin", "Gaming should use Thin LTO");
        assert!(state_after_profile.use_modprobed, "Gaming should enable modprobed");
        assert!(state_after_profile.use_whitelist, "Gaming should enable whitelist");
        ...
        let state_after_overrides = controller.get_state()
            .expect("Failed to get state after overrides");
        
        eprintln!("[TEST] STEP 2 - State after manual overrides:");
        eprintln!("[TEST]   selected_lto: {} (should be full)", state_after_overrides.selected_lto);
        eprintln!("[TEST]   use_modprobed: {} (should be false)", state_after_overrides.use_modprobed);
        eprintln!("[TEST]   use_whitelist: {} (should be false due to modprobed sync)", state_after_overrides.use_whitelist);
        
        // VERIFICATION 2: User overrides are respected (canonical truth)
        assert_eq!(state_after_overrides.selected_lto, "full", "LTO override to full should persist");
        assert!(!state_after_overrides.use_modprobed, "modprobed override should persist");
        assert!(!state_after_overrides.use_whitelist, "whitelist should sync with modprobed");
        ...
        let state_after_variant_change = controller.get_state()
            .expect("Failed to get state after variant change");
        
        eprintln!("[TEST] STEP 3 - State after variant change:");
        eprintln!("[TEST]   selected_variant: {}", state_after_variant_change.selected_variant);
        eprintln!("[TEST]   Previous overrides should persist:");
        eprintln!("[TEST]   selected_lto: {}", state_after_variant_change.selected_lto);
        ...
        assert_eq!(state_after_variant_change.selected_lto, "full", "LTO override should persist after variant change");
        ...
        let state_after_scx = controller.get_state()
            .expect("Failed to get state after SCX change");
        
        eprintln!("[TEST] STEP 4 - State after SCX profile change:");
        eprintln!("[TEST]   selected_scx_profile: {}", state_after_scx.selected_scx_profile);
        eprintln!("[TEST]   Build options should remain untouched:");
        eprintln!("[TEST]   selected_profile: {}", state_after_scx.selected_profile);
        
        // VERIFICATION 4: SCX changes don't corrupt build settings
        assert_eq!(state_after_scx.selected_scx_profile, "Gaming (Burst)");
        assert_eq!(state_after_scx.selected_profile, "Gaming", "Build profile should persist");
        assert_eq!(state_after_scx.selected_lto, "full", "LTO should persist across SCX changes");
        
        // STEP 5: Simulate hardening toggle
        controller.handle_hardening_change("hardened")
            .expect("Failed to toggle hardening");
        
        let state_after_hardening = controller.get_state()
            .expect("Failed to get state after hardening change");
        
        eprintln!("[TEST] STEP 5 - State after hardening toggle:");
        eprintln!("[TEST]   kernel_hardening: {}", state_after_hardening.kernel_hardening);
        
        // VERIFICATION 5: Hardening is applied independently
        assert_eq!(state_after_hardening.kernel_hardening, goatd_kernel::models::HardeningLevel::Hardened, "Hardening should be hardened");
        
        // FINAL COMPREHENSIVE VERIFICATION
        eprintln!("[TEST] === FINAL COMPREHENSIVE STATE SNAPSHOT ===");
        eprintln!("[TEST] Build Configuration:");
        eprintln!("[TEST]   Variant: {}", state_after_hardening.selected_variant);
        eprintln!("[TEST]   Profile: {}", state_after_hardening.selected_profile);
        eprintln!("[TEST]   LTO: {}", state_after_hardening.selected_lto);
        eprintln!("[TEST]   Hardening: {}", state_after_hardening.kernel_hardening);
        ...
        eprintln!("[TEST]   Modprobed: {}", state_after_hardening.use_modprobed);
        eprintln!("[TEST]   Whitelist: {}", state_after_hardening.use_whitelist);
        eprintln!("[TEST] SCX:");
        eprintln!("[TEST]   Profile: {}", state_after_hardening.selected_scx_profile);
        ...
        assert_eq!(state_after_hardening.selected_profile, "Gaming",
                   "Profile remains Gaming throughout test");
        assert_eq!(state_after_hardening.selected_lto, "full",
                   "LTO remains full (user override)");
        assert!(state_after_hardening.use_polly,
