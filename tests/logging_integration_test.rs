...
async fn test_logging_integration_full_cycle() {
    ...
    let collector =
        LogCollector::new(temp_dir.clone(), ui_tx).expect("Failed to initialize LogCollector");
    ...
    assert!(
        !log_files.is_empty(),
        "At least one log file should exist in full directory"
    );
    ...
    let log_content = fs::read_to_string(latest_log.path()).expect("Failed to read log file");

    eprintln!(
        "[TEST] Log file content (first 200 chars): {}",
    ...
    assert!(
        ...
        "At least one parsed log file should exist"
    );

    let parsed_content =
        fs::read_to_string(parsed_files[0].path()).expect("Failed to read parsed log file");
    ...
    let session_content =
        fs::read_to_string(&session_log_path).expect("Failed to read session log file");

    eprintln!(
        "[TEST] Session log content (first 200 chars): {}",
        &session_content[..session_content.len().min(200)]
    );
...
async fn test_log_collector_cloning() {
    ...
    let log_content = fs::read_to_string(log_files[0].path()).expect("Failed to read log file");
...
async fn test_rapid_session_changes() {
    ...
    assert_eq!(
        session_files.len(),
        3,
        "Should have created 3 session log files"
    );

    // Verify each file has content (was written to)
    for (i, file) in session_files.iter().enumerate() {
        let content = fs::read_to_string(file.path()).expect("Failed to read session file");

        assert!(
            !content.is_empty(),
            "Session {} file should contain logs",
            i
        );
        eprintln!(
            "[TEST] Session {} file has {} bytes of data",
            i,
            content.len()
        );
    }
