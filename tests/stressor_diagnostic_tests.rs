...
fn diagnostic_nice_error_handling() {
    ...
    unsafe {
        ...
        if new_nice < 0 {
            eprintln!("[DIAG] ⚠️  WARNING: nice() returned negative value {}", new_nice);
    ...
    manager.start_stressor(StressorType::Cpu, Intensity::new(15))
...
fn diagnostic_cpu_affinity_target_core_zero() {
    ...
    eprintln!("[DIAG] Expected behavior: Stressor CPUs = {{1, 2, ..., {}}}", num_cores - 1);
...
fn diagnostic_drop_with_active_workers() {
    ...
    
    {
        ...
        eprintln!("[DIAG] Manager has {} active workers", manager.active_count());
...
fn diagnostic_iteration_counter_accuracy() {
    ...
    manager.start_stressor(StressorType::Cpu, Intensity::new(10))
        .expect("Failed to start CPU stressor");
    ...
    eprintln!("[DIAG] Iteration progress: {} in first 100ms, {} in next 100ms", diff1, diff2);
