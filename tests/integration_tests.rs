...
async fn test_orchestrator_instantiation() {
    ...
    let result = AsyncOrchestrator::new(
        hw,
        config,
...
async fn test_phase_transition_preparation_to_configuration() {
    ...
    assert!(
        result.is_ok(),
        "Should transition from Preparation to Configuration"
    );
...
async fn test_phase_transition_configuration_to_patching() {
    ...
    assert!(
        result.is_ok(),
        "Should transition from Configuration to Patching"
    );
...
async fn test_patch_tracking() {
    ...
    let snapshot = orch.state_snapshot().await;
    assert_eq!(
        snapshot.patches_applied, 2,
        "Should track successful patches"
    );
...
async fn test_recovery_from_failed_state() {
    ...
    assert!(
        result.is_ok(),
        "Should allow transition from Failed to Preparation for recovery"
    );
    assert_eq!(orch.current_phase().await, BuildPhaseState::Preparation);
}
...
async fn test_recovery_flag_management() {
    ...
    assert!(
        !orch.is_recovery_enabled(),
        "Recovery should be disabled after setting"
    );

    orch.set_recovery_enabled(true);
    assert!(
        orch.is_recovery_enabled(),
        "Recovery should be enabled again"
    );
}
