...
async fn test_install_pipe_lifecycle_kernel() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!(
        "[TEST] Created test logs directory: {}",
        test_logs_dir.display()
    );
    ...
    let init_result = log_collector
        .clone()
        .init_global_logger(log::LevelFilter::Info);
    ...
    let session_log_path = log_collector
        .start_new_session("lifecycle_test.log")
        .await?;
    println!(
        "[TEST] ✓ Started new logging session: {}",
    ...
    println!(
        "[TEST] ✓ Retrieved session log ({} bytes)",
        session_log_content.len()
    );
    ...
    // =========================================================================
    let kernel_logs = session_log_content
        .lines()
    ...
    if installation_success {
        ...
        println!(
            "[TEST] Attempting uninstallation of: {} ({})",
            kernel_to_install.name, kernel_to_install.version
        );
    ...
    println!(
        "[TEST] ✓ Installation async execution: {}",
        if installation_completed {
            "PASS"
        } else {
            "FAIL"
...
async fn test_log_collector_initialization() {
    ...
    if init_result.is_ok() {
        ...
        println!(
            "[TEST] ℹ️  LogCollector init skipped (logger already registered from previous test)"
        );
    }
...
fn test_kernel_manager_scan_workspace() {
    ...
    println!(
        "[TEST] Found {} kernel artifacts in workspace",
        kernels.len()
    );
