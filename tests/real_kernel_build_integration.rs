...
async fn test_async_orchestrator_timeout_with_log_capture() {
    ...
    log_collector.log_str(
        "[LOG-CAPTURE] AsyncOrchestrator initialized with 5-second timeout",
    );
    ...
    // ========================================================================
    log_collector.log_str(
        "[LOG-CAPTURE] Attempting to run build - expected to fail or timeout",
    );
    ...
    if let Err(err) = &build_result {
        ...
        log_collector.log_str(
            format!("[LOG-CAPTURE] Build error captured: {}", error_msg),
        );
        eprintln!("[TEST] Build failed as expected: {}", error_msg);
    } else {
        // Build unexpectedly succeeded - might happen if sources exist
        eprintln!(
            "[TEST] Build completed (sources may have existed in test environment)"
        );
    }
    ...
    assert!(
        full_log_dir.exists(),
        "Full log directory should exist"
    );
    assert!(
        parsed_log_dir.exists(),
        "Parsed log directory should exist"
    );
    ...
    // ========================================================================
    log_collector.log_str(
        "[LOG-CAPTURE] === TIMEOUT INTEGRATION TEST COMPLETED ===",
    );
...
async fn test_build_pipe_lifecycle_gaming() {
   ...
   eprintln!("[FULL-BUILD-PIPE] Configuration: linux-mainline + Gaming + Full LTO + Minimal Hardening");
   ...
   eprintln!("[FULL-BUILD-PIPE] AsyncOrchestrator creation result: {:?}", orch_result.is_ok());
   ...
   log_collector.log_str(
       "[FULL-BUILD-PIPE] AsyncOrchestrator initialized with 5-second timeout",
   );
   ...
   // ========================================================================
   log_collector.log_str(
       "[FULL-BUILD-PIPE] Attempting to run build - will timeout or fail",
   );
   ...
   eprintln!("[FULL-BUILD-PIPE] Build execution completed in {:?}", elapsed);
   log_collector.log_str(
       format!("[FULL-BUILD-PIPE] Build execution completed in {:?}", elapsed),
   );
   ...
   if let Err(err) = &build_result {
       let error_msg = err.to_string();
       log_collector.log_str(
           format!("[FULL-BUILD-PIPE] Build error captured: {}", error_msg),
       );
   ...
   // Flush all logs to disk
   let flush_result = log_collector.wait_for_empty().await;
   eprintln!("[FULL-BUILD-PIPE] Log flush result: {:?}", flush_result.is_ok());
   ...
   eprintln!("[FULL-BUILD-PIPE] Full log dir exists: {}", full_log_dir.exists());
   eprintln!("[FULL-BUILD-PIPE] Parsed log dir exists: {}", parsed_log_dir.exists());
   ...
   // ========================================================================
   log_collector.log_str(
       "[FULL-BUILD-PIPE] === FULL BUILD PIPE TEST COMPLETED ===",
   );
