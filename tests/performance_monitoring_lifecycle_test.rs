...
fn test_session_summary_initialization() {
    ...
    println!(
        "[TEST] ✓ SessionSummary.mark_completed() set duration={:.3}s",
        duration
    );
}
...
fn test_session_summary_sample_capture() {
    ...
    assert_eq!(
        summary.total_samples, 5000,
        "Sample count mismatch - CRITICAL BUG"
    );
    assert_eq!(
        summary.total_dropped_samples, 50,
        "Dropped count mismatch - CRITICAL BUG"
    );
...
fn test_performance_metrics_clone() {
    ...
    assert_eq!(
        metrics1.spikes_correlated_to_smi,
        metrics2.spikes_correlated_to_smi
    );
...
fn test_latency_processor_sample_counting() {
    ...
    assert!(avg > 0.0);
    println!(
        "[TEST] ✓ LatencyProcessor percentiles: p99={:.2}µs, p99.9={:.2}µs, avg={:.2}µs",
        p99, p99_9, avg
    );
}
...
fn test_performance_config_defaults() {
    ...
    println!(
        "[TEST] ✓ Default interval_us: {} µs (1 ms)",
        config.interval_us
    );
    ...
    println!(
        "[TEST] ✓ Default spike_threshold_us: {} µs",
        config.spike_threshold_us
    );
}
