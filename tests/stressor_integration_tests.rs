...
use goatd_kernel::system::performance::stressor::{StressorManager, StressorType, Intensity};
...
fn test_stressor_spawn_and_graceful_stop() {
    ...
    manager.start_stressor(StressorType::Cpu, Intensity::new(30))
    ...
    assert!(iter_count > 0, "CPU stressor should have completed iterations");
    
    // Stop gracefully
    eprintln!("[TEST] Stopping all stressors");
    manager.stop_all_stressors().expect("Failed to stop stressors");
...
fn test_stressor_drop_cleanup() {
    ...
    
    {
        ...
        manager.start_stressor(StressorType::Cpu, Intensity::new(25))
            .expect("Failed to start CPU stressor");
...
fn test_stressor_multiple_types() {
    ...
    eprintln!("[TEST] Spawning CPU stressor");
    manager.start_stressor(StressorType::Cpu, Intensity::new(20))
        .expect("Failed to start CPU stressor");
    
    eprintln!("[TEST] Spawning Memory stressor");
    manager.start_stressor(StressorType::Memory, Intensity::new(20))
        .expect("Failed to start Memory stressor");
    
    eprintln!("[TEST] Spawning Scheduler stressor");
    manager.start_stressor(StressorType::Scheduler, Intensity::new(25))
...
fn test_stressor_duplicate_type_prevention() {
    ...
    assert_eq!(manager.active_count(), 1, "Should still have only 1 stressor");
...
fn test_stressor_after_stop_rejection() {
    ...
    assert!(result.is_err(), "Should reject starting stressor after stop_all_stressors");
...
fn test_stressor_high_intensity_thundering_herd() {
    ...
    manager.start_stressor(StressorType::Scheduler, Intensity::new(100))
...
fn test_stressor_invalid_core_rejection() {
    ...
    eprintln!("[TEST] Attempting to create manager with invalid core {}", invalid_core);
