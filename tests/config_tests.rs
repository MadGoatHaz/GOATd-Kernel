...
fn test_load_valid_json_profile() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert_eq!(loaded.version, "latest");
...
fn test_invalid_lto_type_detected() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for lto in lto_types {
        ...
        assert!(
            validator::validate_config(&config).is_ok(),
            "Config with valid LTO should pass"
        );
    }
...
fn test_conflicting_options_detected() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert!(
        result.is_err(),
        "Full LTO with many exclusions should be detected as conflict"
    );
    ...
    assert!(
        result2.is_err(),
        "Whitelist + exclusions should be detected as conflict"
    );
...
fn test_missing_essential_fields_handled() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert!(
        result.is_err(),
        "Empty kernel version should fail validation"
    );
    ...
    assert!(
        result2.is_err(),
        "Empty option value should fail validation"
    );
...
fn test_whitelist_validation_succeeds() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert!(
        result.is_ok(),
        "Config with non-essential exclusions should pass whitelist validation"
    );
...
fn test_whitelist_validation_fails() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert!(
        result.is_err(),
        "Config with essential driver exclusions should fail validation"
    );

    if let Err(e) = result {
        let msg = e.to_string();
        assert!(
            msg.contains("i915") || msg.contains("ext4"),
            "Error should mention excluded essential drivers"
        );
    }
...
fn test_valid_exclusions_applied() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert!(config
        .driver_exclusions
        .contains(&"custom_driver".to_string()));
    assert!(config
        .driver_exclusions
        .contains(&"another_driver".to_string()));
...
fn test_exclude_essential_driver_fails() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for driver in essential_drivers {
        ...
        assert!(
            result.is_err(),
            "Should not be able to exclude essential driver: {}",
            driver
        );
    }
...
fn test_multiple_exclusions_handled() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert!(
        result.is_err(),
        "Should fail when trying to exclude essential driver"
    );
    assert_eq!(
        config.driver_exclusions.len(),
        4,
        "driver4 should have been added before error"
    );
...
fn test_very_large_config_objects() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for i in 0..1000 {
        options.insert(format!("CONFIG_OPTION_{}", i), format!("value_{}", i));
    }
    ...
    assert!(
        result.is_ok(),
        "Config with 1000 options and Thin LTO should validate"
    );
    ...
    assert_eq!(
        loaded.config_options.len(),
        1000,
        "All 1000 options should be preserved"
    );
...
fn test_config_manager_integration() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert_eq!(
        manager.get_config_option("CONFIG_FOO"),
        Some(&"bar".to_string())
    );
...
fn test_modprobed_integration_with_config() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let used_modules: HashSet<String> = vec!["nouveau"].iter().map(|s| s.to_string()).collect();
...
fn test_whitelist_protection_integration() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert!(
        result.is_ok(),
        "After applying whitelist, validation should pass"
    );
...
fn test_variant_urls_are_valid_git_endpoints() {
    ...
    for variant in variants {
        if let Some(url) = get_variant_repo_url(variant) {
            assert!(
                url.ends_with(".git"),
                "URL for {} should end with .git",
                variant
            );
            assert!(
                url.starts_with("https://"),
                "URL for {} should use https",
                variant
            );
        }
    }
...
fn test_storage_detection_longest_prefix_nested() {
    ...
    assert_eq!(
        best,
        Some(3),
        "Should match /home/user/workspace (longest prefix)"
    );
}
...
fn test_storage_detection_multiple_similar_prefixes() {
    ...
    assert_eq!(
        best,
        Some(2),
        "Should match /mnt/nvme (longest matching prefix)"
    );
}
