...
async fn main() -> goatd_kernel::Result<()> {
    ...
    {
        eprintln!(
            "[Main] WARNING: Failed to set LogCollector as global logger: {}",
            e
        );
    } else {
        eprintln!(
    ...
    // NOTE: Bridge task reconnects log_ui_rx to build_tx for UI event processing
    // This pipes LogCollector output directly to the AppUI event loop
    let build_tx_bridge = build_tx.clone();
    tokio::spawn(async move {
        let mut rx = log_ui_rx;
        while let Some(log_line) = rx.recv().await {
            let _ = build_tx_bridge.send(BuildEvent::Log(log_line.message)).await;
        }
    });

    let (cancel_tx, _cancel_rx) = tokio::sync::watch::channel(false);

    let controller = AppController::new_production(
        build_tx.clone(),
        cancel_tx.clone(),
    ...
    let hw_cache_clone = controller.clone().read().await.cached_hardware_info.clone();
    let hw_ts_clone = controller
        .clone()
        .read()
        .await
        .hardware_cache_timestamp
        .clone();
    ...
    tokio::spawn(async move {
        ...
        match detector.detect_all() {
            Ok(hw_info) => {
                eprintln!(
                    "[Main] [HW] ✓ Detected: CPU={}, RAM={}GB, GPU={:?}",
    ...
    let result = eframe::run_native(
        "GOATd Kernel",
        options,
        Box::new(move |_cc| Box::new(app_ui)),
    );
    ...
    if let Err(e) = log_collector.wait_for_empty().await {
        eprintln!(
            "[Main] WARNING: Failed to wait for log collector to empty: {}",
            e
        );
    } else {
