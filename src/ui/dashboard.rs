...
pub fn render_dashboard(
    ...
    ui_state: &mut super::app::UIState,
) {
    // SCROLLABLE CONTAINER: Wrap entire dashboard to handle vertical overflow at any scale
    egui::ScrollArea::vertical()
        .auto_shrink([false; 2])
        .show(ui, |ui| {
            render_dashboard_content(ui, controller, ui_state);
        });
}

/// Helper function containing the actual dashboard content
fn render_dashboard_content(
    ...
    let hardware_info = {
        match controller.try_read() {
            Ok(controller_guard) => controller_guard.get_hardware_info(),
            Err(_) => {
    ...
    // Create a responsive hardware grid using StripBuilder
    // Check available width to determine layout
    let available_width = ui.available_width();
    let use_single_column = available_width < 1000.0;

    if use_single_column {
        // Single column layout for narrow views
        egui::Grid::new("hardware_grid_single")
            .spacing([20.0, 8.0])
            .striped(true)
            .show(ui, |ui| {
                // CPU Information
        ...
        // Two-column layout for wider views using StripBuilder
        render_hardware_grid_two_column(ui, &hardware_info);
    }
    ...
    if !health_report.missing_official_packages.is_empty() {
        ui.colored_label(
            ...
            format!(
                ...
                health_report.missing_official_packages.join(", ")
            ),
        );
    }
    ...
    if !health_report.missing_gpg_keys.is_empty() {
        ui.colored_label(
            ...
            format!(
                "Missing GPG keys: {}",
                health_report.missing_gpg_keys.join(", ")
            ),
        );
    }
    ...
    if !health_report.missing_aur_packages.is_empty() {
        ui.colored_label(
            ...
            format!(
                ...
                health_report.missing_aur_packages.join(", ")
            ),
        );
        ui.colored_label(
            egui::Color32::from_rgb(200, 150, 100),
            "  → Install manually: yay -S ".to_string()
                + &health_report.missing_aur_packages.join(" "),
        );
    }
    ...
    if !health_report.missing_optional_tools.is_empty() {
        ui.colored_label(
            ...
            format!(
                "Optional: {}",
                health_report.missing_optional_tools.join(", ")
            ),
        );
    }
    ...
    if !health_report.missing_official_packages.is_empty()
        || !health_report.missing_gpg_keys.is_empty()
        || !health_report.missing_optional_tools.is_empty()
    {
        ui.add_space(4.0);
        if ui.button("🔧 Fix System Environment (pkexec)").clicked() {
            let fix_cmd =
                crate::system::health::HealthManager::generate_fix_command(&health_report);
    ...
    // Recent Audit Results Section - Responsive Layout with StripBuilder
    ui.heading("Recent Audit Results");

    // Determine layout based on available width
    let available_width = ui.available_width();
    let use_two_columns = available_width >= 1000.0;

    if use_two_columns {
        // Two-column horizontal layout using StripBuilder
        render_audit_results_two_column(ui, controller);
    } else {
        // Single-column vertical layout
        render_audit_results_single_column(ui, controller);
    }
...
/// Helper function to render hardware grid in two-column layout using StripBuilder
fn render_hardware_grid_two_column(ui: &mut egui::Ui, hardware_info: &Result<HardwareInfo, String>) {
    let available_width = ui.available_width();
    let height_constraint = 160.0;
    
    // Use allocate_ui to reserve space and advance parent cursor
    ui.allocate_ui(egui::vec2(available_width, height_constraint), |ui| {
        if available_width > 1700.0 {
            StripBuilder::new(ui)
                .size(Size::remainder())
                .size(Size::initial(500.0))
                .size(Size::initial(500.0))
                .size(Size::remainder())
                .horizontal(|mut strip| {
                    strip.cell(|_ui| {}); // Left spacer (removed greedy sizing)
                    
                    strip.cell(|ui| {
                        egui::Grid::new("hardware_grid_left")
                            .spacing([20.0, 8.0])
                            .striped(true)
                            .show(ui, |ui| {
                                // CPU Information
                    ...
                    strip.cell(|ui| {
                        egui::Grid::new("hardware_grid_right")
                            .spacing([20.0, 8.0])
                            .striped(true)
                            .show(ui, |ui| {
                                // Storage Information
...
/// Helper function to render audit results in two-column layout using StripBuilder
fn render_audit_results_two_column(
    ui: &mut egui::Ui,
    controller: &Arc<RwLock<AppController>>,
) {
    let available_width = ui.available_width();
    let height_constraint = 300.0;
        ...
        if available_width > 1700.0 {
            StripBuilder::new(ui)
                ...
                .horizontal(|mut strip| {
                    ...
                    // COLUMN 1: Jitter Audit Results
                    strip.cell(|ui| {
                        ui.group(|ui| {
                            ui.label("Jitter Audit");

                            match controller.try_read() {
                                Ok(controller_guard) => match controller_guard.get_cached_jitter_summary() {
                                    Ok(Some(summary)) => {
...
/// Helper function to render audit results in single-column layout
fn render_audit_results_single_column(
    ui: &mut egui::Ui,
    controller: &Arc<RwLock<AppController>>,
) {
    // Deep Audit Results
    ui.group(|ui| {
        ...
        match controller.try_read() {
            Ok(controller_guard) => match controller_guard.get_active_audit() {
                Ok(audit_data) => {
                    egui::Grid::new("deep_audit_grid_single")
                        .spacing([20.0, 8.0])
                        .striped(true)
                        .show(ui, |ui| {
                            ui.label("Kernel Version:");
    ...
    ui.group(|ui| {
        ...
        match controller.try_read() {
            Ok(controller_guard) => match controller_guard.get_cached_jitter_summary() {
                Ok(Some(summary)) => {
                    egui::Grid::new("jitter_audit_grid_single")
                        .spacing([20.0, 8.0])
                        .striped(true)
                        .show(ui, |ui| {
                            ui.label("Min Latency (µs):");
...
// ============================================================================
// CACHED HEALTH REPORT WITH 60S TTL
// ============================================================================

pub fn get_cached_health_report(
    cached_health_report: &Arc<std::sync::RwLock<Option<crate::system::health::HealthReport>>>,
    health_check_timestamp: &Arc<std::sync::RwLock<Option<Instant>>>,
) -> crate::system::health::HealthReport {
    let now = Instant::now();
    let cache_duration = Duration::from_secs(60);

    // FAST PATH: Check if cache is valid - return immediately
    if let Ok(timestamp_lock) = health_check_timestamp.read() {
        if let Some(last_check) = *timestamp_lock {
            if now.duration_since(last_check) < cache_duration {
                ...
                if let Ok(cache_lock) = cached_health_report.read() {
    ...
    let _ = cached_health_report.write().map(|mut cache| {
        *cache = Some(report.clone());
    });
    let _ = health_check_timestamp.write().map(|mut ts| {
        *ts = Some(now);
