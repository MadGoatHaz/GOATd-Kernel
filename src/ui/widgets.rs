...
pub fn radial_gauge(ui: &mut egui::Ui, value: f32, range: std::ops::Range<f32>, label: &str) {
    // Reserve space for gauge + label below (140px total height for proper spacing)
    let (response, painter) = ui.allocate_painter(Vec2::new(120.0, 140.0), egui::Sense::hover());
    ...
    painter.circle_stroke(center, radius, Stroke::new(3.0, Color32::from_gray(100)));
...
/// * `label` - Display label
pub fn sparkline(ui: &mut egui::Ui, samples: &[f32], label: &str) {
    if samples.is_empty() {
        ui.label(label);
        return;
    }
    ...
    let value_to_y = |val: f32| -> f32 { rect.bottom() - ((val - min_val) / range) * height };
    ...
    for i in 1..samples.len() {
        ...
        painter.line_segment([Pos2::new(x0, y0), Pos2::new(x1, y1)], line_stroke);
    }
...
pub fn cpu_heatmap(ui: &mut egui::Ui, core_temps: &[f32], _label: &str) {
...
pub fn terminal_viewport(ui: &mut egui::Ui, log_content: &str) {
