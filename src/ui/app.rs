...
pub struct AppUI {
    ...
    /// Current scale factor (used for hysteresis in resolve_scaling)
    pub current_scale: f32,

    /// Last time scaling was updated (for debouncing in resolve_scaling)
    pub last_scale_update: std::time::Instant,
}
...
impl AppUI {
    ...
    /// Resolve scaling with V2 stabilization (physical pixels, debouncing, hysteresis)
    /// Uses physical pixels as baseline to prevent circular dependency on logical points.
    /// Debounces updates (500ms) and applies hysteresis (0.05 threshold) to prevent pulsing.
    fn resolve_scaling(&mut self, ctx: &egui::Context) {
        if let Ok(guard) = self.controller.try_read() {
            if let Ok(state) = guard.get_state() {
                // Check debounce: only proceed if 500ms has elapsed since last update
                if self.last_scale_update.elapsed() < std::time::Duration::from_millis(500) {
                    return;
                }

                // 1. Get physical width using pixels_per_point for stable reference
                let screen_width = ctx.input(|i| i.screen_rect().size().x);
                let pixels_per_point = ctx.pixels_per_point();
                let physical_width = screen_width * pixels_per_point;

                // 2. Calculate base scale component from physical width (1280px baseline)
                let base_scale = physical_width / 1280.0;

                // 3. Calculate font modifier component
                let font_modifier = if state.ui_font_size > 0.0 {
                    (state.ui_font_size as f32) / 10.4
                } else {
                    1.0
                };

                // 4. Calculate target scale and clamp to valid range
                let target_scale = (base_scale * font_modifier).clamp(0.7, 2.0);

                // 5. Apply hysteresis check: only update if delta > 0.05
                if (target_scale - self.current_scale).abs() > 0.05 {
                    ctx.set_pixels_per_point(target_scale);
                    self.current_scale = target_scale;
                    self.last_scale_update = Instant::now();
                    ctx.request_repaint();
                }
            }
        }
                ...
                match event {
                            ...
                            if let Ok(controller) = self.controller.try_read() {
                                if let Some(ref log_collector) = controller.log_collector {
                                    if let Some(path) = log_collector.get_session_log_path() {
                                        ...
                                        log::debug!(
                                            "[BUILD] [SESSION] Captured log path for UI: {}",
                    ...
                    crate::ui::controller::BuildEvent::InstallationComplete(success) => {
                        if success {
                            ...
                            self.ui_state.error_message = Some(
                                "Kernel installation failed. Check the log for details."
                                    .to_string(),
                            );
                        }
                        self.ui_state.needs_repaint = true;
                    ...
                    crate::ui::controller::BuildEvent::JitterAuditComplete(summary) => {
                        ...
                        log::debug!(
                            "[UI] Jitter audit completed: samples={}, duration={:.2}s",
                    ...
                    crate::ui::controller::BuildEvent::WorkspaceChanged => {
                        // Workspace path changed, reset UI state to force re-scan of kernels
                        self.ui_state.ui_state_initialized = false;
                        eprintln!("[UI] WorkspaceChanged event received, resetting UI state for re-scan");
                        self.ui_state.needs_repaint = true;
                    }
                }
            }
    ...
    fn clear_transient_messages(&mut self, _ctx: &egui::Context) {
        ...
        if self.ui_state.error_message.is_some() || self.ui_state.success_message.is_some() {
    ...
    fn render_messages(&mut self, ctx: &egui::Context) {
        ...
        if let Some(ref msg) = self.ui_state.success_message.clone() {
            egui::TopBottomPanel::top("success_panel").show(ctx, |ui| {
                ui.colored_label(egui::Color32::from_rgb(100, 255, 100), format!("{}", msg));
                if ui.button("Dismiss").clicked() {
        ...
        if let Some(ref msg) = self.ui_state.info_message.clone() {
            egui::TopBottomPanel::top("info_panel").show(ctx, |ui| {
                ui.colored_label(egui::Color32::from_rgb(100, 150, 255), format!("{}", msg));
                if ui.button("Dismiss").clicked() {
            ...
            match self.ui_state.active_tab {
                ...
                Tab::Settings => {
                    super::settings::render_settings(
                        ui,
                        &mut self.ui_state.settings_ui_state,
                        &controller,
                    );
                }
            }
    ...
    fn sync_build_settings(&mut self) {
        ...
        if let Ok(guard) = self.controller.try_read() {
            if let Ok(state) = guard.get_state() {
                ...
                let variant_options =
                    vec!["linux", "linux-lts", "linux-hardened", "linux-mainline"];
                if let Some(variant_idx) = variant_options
                    .iter()
                    .position(|&v| v == state.selected_variant)
                {
                ...
                if let Some(profile_idx) = profile_options
                    .iter()
                ...
                if let Some(lto_idx) = lto_options.iter().position(|&l| l == state.selected_lto) {
                    if self.ui_state.selected_lto != lto_idx {
                        log::debug!(
                            "[SYNC] LTO mismatch: UI index={}, Backend='{}'. Updating UI state.",
                            self.ui_state.selected_lto,
                            state.selected_lto
                        );
                ...
                if self.ui_state.selected_hardening != hardening_idx {
                    log::debug!(
                        "[SYNC] Hardening mismatch: UI index={}, Backend={:?}. Updating UI state.",
                        self.ui_state.selected_hardening,
                ...
                if self.ui_state.use_polly != state.use_polly {
                    log::debug!(
                        "[SYNC] Polly mismatch: UI={}, Backend={}. Updating UI state.",
                        self.ui_state.use_polly,
                        state.use_polly
                    );
                ...
                if self.ui_state.use_mglru != state.use_mglru {
                    log::debug!(
                        "[SYNC] MGLRU mismatch: UI={}, Backend={}. Updating UI state.",
                        self.ui_state.use_mglru,
                        state.use_mglru
                    );
                ...
                if self.ui_state.use_modprobed != state.use_modprobed {
                    log::debug!(
                        "[SYNC] Modprobed mismatch: UI={}, Backend={}. Updating UI state.",
                        self.ui_state.use_modprobed,
                        state.use_modprobed
                    );
                ...
                if self.ui_state.use_whitelist != state.use_whitelist {
                    log::debug!(
                        "[SYNC] Whitelist mismatch: UI={}, Backend={}. Updating UI state.",
                        self.ui_state.use_whitelist,
                        state.use_whitelist
                    );
...
impl eframe::App for AppUI {
    fn update(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
        ...
        // IDEMPOTENT SCALING: Resolve scaling with hysteresis to break feedback loop
        // This consolidates width-based and font-based scaling into a single call
        // with hysteresis (threshold 0.01) to prevent oscillation
        self.resolve_scaling(ctx);
        ...
        // =====================================================================
        // This consolidated strategy ensures consistent UI updates across all tabs
        // without requiring mouse movement or manual interaction.
        //
        // Strategy:
        // 1. Check both atomic_perf_dirty AND atomic_ui_dirty flags (lock-free signaling)
        // 2. Compute high-activity state from task flags (building, auditing)
        // 3. Use 100ms polling during high activity, 500ms during idle
        // 4. All tabs benefit from unified signaling (no tab-specific special cases)
        
        const HIGH_ACTIVITY_INTERVAL_MS: u64 = 100;  // Poll every 100ms when tasks active
        const IDLE_REPAINT_INTERVAL_MS: u64 = 500;   // Fallback every 500ms when idle
        let elapsed_since_last_repaint = self.ui_state.last_repaint_time.elapsed();

        // Check both atomic flags: performance updates AND generic UI state changes
        let (perf_data_dirty, ui_data_dirty) = if let Ok(controller) = self.controller.try_read() {
            let perf_dirty = controller
                .atomic_perf_dirty
                .load(std::sync::atomic::Ordering::Acquire);
            let ui_dirty = controller
                .check_ui_dirty();  // Atomically load and clear the flag
            (perf_dirty, ui_dirty)
        } else {
            (false, false)
        };

        // Compute high-activity state: true if any background task is active
        let high_activity = self.ui_state.is_building
            || self.ui_state.is_auditing_deep
            || self.ui_state.is_auditing_jitter;

        // Determine polling interval based on activity
        let poll_interval_ms = if high_activity {
            HIGH_ACTIVITY_INTERVAL_MS
        } else {
            IDLE_REPAINT_INTERVAL_MS
        };

        // Unified repaint logic: signal based on event OR idle timeout OR activity polling
        if self.ui_state.needs_repaint || perf_data_dirty || ui_data_dirty {
            // Data changed: request immediate repaint (event-driven)
            ctx.request_repaint();
            self.ui_state.needs_repaint = false;
            self.ui_state.last_repaint_time = Instant::now();
        } else if elapsed_since_last_repaint.as_millis() > poll_interval_ms as u128 {
            // Poll timeout: request repaint at activity-based interval
        ...
        let is_scx_check_active_tab = matches!(
            self.ui_state.active_tab,
            Tab::Dashboard | Tab::KernelManager
        );
        ...
        if !self.ui_state.scx_initial_sync_done
            && !self.ui_state.available_scx_schedulers.is_empty()
        {
            ...
            if let Some(idx) = self
                ...
                .iter()
                .position(|s| s == &target_scheduler)
            {
                self.ui_state.selected_scx_type_idx = Some(idx);
                log::debug!(
                    "[SCX SYNC] Initial sync: matched target scheduler '{}' at index {}",
                    target_scheduler,
                    idx
                );
            } else {
