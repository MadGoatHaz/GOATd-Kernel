...
fn expected_with_page_free(original: &str) -> String {
    ...
    format!("{}\n\tvoid (*page_free)(struct page *page);", original.trim_end())
}
...
fn test_memremap_standard_restoration() {
    ...
    fs::write(&memremap_path, &original_content)
        .expect("Failed to write test file");
    
    eprintln!("[TEST] standard_restoration: Created test file at {:?}", memremap_path);
    ...
    let modified_content = fs::read_to_string(&memremap_path)
        .expect("Failed to read modified file");
    
    assert!(modified_content.contains("page_free"), 
...
fn test_memremap_idempotency() {
    ...
    eprintln!("[TEST] idempotency: Created test file at {:?}", memremap_path);
    ...
    let after_first = fs::read_to_string(&memremap_path)
        .expect("Failed to read after first execution");
    ...
    let after_second = fs::read_to_string(&memremap_path)
        .expect("Failed to read after second execution");
...
fn test_memremap_tier2_fallback() {
    ...
    fs::write(&memremap_path, &malformed_content)
        .expect("Failed to write test file");
    
    eprintln!("[TEST] tier2_fallback: Created test file with malformed struct at {:?}", memremap_path);
    ...
    let after_perl = fs::read_to_string(&memremap_path)
        .expect("Failed to read after Perl execution");
    
    let has_page_free_from_perl = after_perl.contains("page_free");
    eprintln!("[TEST] tier2_fallback: Perl injection result: page_free present = {}", has_page_free_from_perl);
    
    // STEP 4: If Perl failed (as expected on malformed struct), apply Tier 2 fallback
    if !has_page_free_from_perl {
        eprintln!("[TEST] tier2_fallback: Perl injection failed/skipped - initiating Tier 2 fallback");
...
"#;
        ...
        fs::write(&memremap_path, &content)
            .expect("Failed to write Tier 2 fallback");
...
fn test_memremap_complete_workflow() {
    ...
    assert!(original_content.contains("struct dev_pagemap_ops"),
    ...
    let modified = fs::read_to_string(&memremap_path)
        .expect("Failed to read modified file");
    
    assert!(modified.contains("page_free"), "page_free field not found in result");
    
    // Debug: Print what we got
    eprintln!("[TEST] complete_workflow: Modified content preview:\n{}",
        modified.lines().skip_while(|l| !l.contains("struct dev_pagemap_ops")).take(10).collect::<Vec<_>>().join("\n"));
...
fn test_headers_regex_pattern_matching() {
    ...
    let headers_regex = Regex::new(r"(?m)^package_.*?headers\s*\(\)\s*\{")
...
"#;
    ...
    for (i, expected) in expected_patterns.iter().enumerate() {
        ...
        eprintln!("[TEST] headers_regex: ✓ Match {}: {}", i + 1, match_strs[i].trim());
    }
    
    // STEP 4: Verify the regex DOES NOT match non-headers functions
    let non_headers_patterns = [
        "package_linux-mainline() {",
        "package() {",
        "_package() {",
    ];
    ...
    for pattern in &non_headers_patterns {
        let test_content = mock_pkgbuild.replace("package_linux-mainline-goatd-gaming-headers", "package_test");
        if test_content.contains(pattern) {
            ...
            eprintln!("[TEST] headers_regex: ✓ Correctly rejected non-headers pattern: {}", pattern);
        }
    }
...
fn test_pkgbase_regex_pattern_matching() {
    ...
    if let Some(caps) = pkgbase_regex.captures(test1_content) {
        let variant = caps.get(1).or_else(|| caps.get(2))
            .map(|m| m.as_str().trim())
            .unwrap_or("");
        assert_eq!(variant, "linux-mainline",
            "Test 1 failed: Expected 'linux-mainline', got '{}' for {}",
            variant, test1_content);
        eprintln!("[TEST] pkgbase_regex: ✓ Test 1 PASSED - {} -> {}", test1_content, variant);
    } else {
    ...
    if let Some(caps) = pkgbase_regex.captures(test2_content) {
        ...
        eprintln!("[TEST] pkgbase_regex: ✓ Test 2 PASSED - {} -> {}", test2_content, variant);
    } else {
    ...
    if let Some(caps) = pkgbase_regex.captures(test3_content) {
        ...
        eprintln!("[TEST] pkgbase_regex: ✓ Test 3 PASSED - {} -> {}", test3_content, variant);
    } else {
    ...
    if let Some(caps) = pkgbase_regex.captures(test4_content) {
        let variant = caps.get(1).or_else(|| caps.get(2))
            .map(|m| m.as_str().trim())
            .unwrap_or("");
        assert_eq!(variant, "linux-zen",
            "Test 4 failed: Expected 'linux-zen', got '{}' for {}",
            variant, test4_content);
        eprintln!("[TEST] pkgbase_regex: ✓ Test 4 PASSED - {} -> {}", test4_content, variant);
    } else {
    ...
    if let Some(caps) = pkgbase_regex.captures(test5_content) {
        ...
        eprintln!("[TEST] pkgbase_regex: ✓ Test 5 PASSED - {} -> {}", test5_content, variant);
    } else {
    ...
    if let Some(caps) = pkgbase_regex.captures(test6_content) {
        let variant = caps.get(1).or_else(|| caps.get(2))
            .map(|m| m.as_str().trim())
            .unwrap_or("");
        assert_eq!(variant, "linux",
            "Test 6 failed: Expected 'linux', got '{}' for {}",
            variant, test6_content);
        eprintln!("[TEST] pkgbase_regex: ✓ Test 6 PASSED - {} -> {}", test6_content, variant);
    } else {
    ...
    if let Some(caps) = pkgbase_regex.captures(test7_content) {
        ...
        eprintln!("[TEST] pkgbase_regex: ✓ Test 7 PASSED - {} -> {}", test7_content, variant);
    } else {
...
"#;
    ...
    let rebranded_content = fs::read_to_string(&pkgbuild_path).expect("Failed to read rebranded PKGBUILD");
...
"#;
   ...
   patch_pkgbuild_for_rebranding(src_dir, "gaming")
       .expect("First rebranding failed");
   
   let first_branded = fs::read_to_string(&pkgbuild_path)
       .expect("Failed to read first rebranded PKGBUILD");
   
   eprintln!("[TEST] rebranding_idempotency: First rebranding complete");
   eprintln!("[TEST] rebranding_idempotency: First branded content:\n{}\n", first_branded);
   
   // STEP 2: Second rebranding (gaming -> gaming, should be idempotent)
   patch_pkgbuild_for_rebranding(src_dir, "gaming")
       .expect("Second rebranding failed");
   
   let second_branded = fs::read_to_string(&pkgbuild_path)
       .expect("Failed to read second rebranded PKGBUILD");
   
   eprintln!("[TEST] rebranding_idempotency: Second rebranding complete");
   eprintln!("[TEST] rebranding_idempotency: Second branded content:\n{}\n", second_branded);
   
   // STEP 3: Verify idempotency - both should be identical
   assert_eq!(first_branded, second_branded,
   ...
   assert!(!second_branded.contains("linux-zen-goatd-goatd-gaming"),
       "DOUBLE-BRANDING ERROR: Found double-branded 'linux-zen-goatd-goatd-gaming'");
   assert!(!second_branded.contains("gaming-gaming"),
   ...
   assert!(second_branded.contains("pkgbase='linux-zen-goatd-gaming'"),
       "Expected pkgbase not found");
   assert!(second_branded.contains("'linux-zen-goatd-gaming'"),
       "Expected branding not found in pkgname");
   assert!(second_branded.contains("'linux-zen-goatd-gaming-headers'"),
       "Expected headers branding not found in pkgname");
   assert!(second_branded.contains("package_linux-zen-goatd-gaming() {"),
       "Expected package function not found");
   assert!(second_branded.contains("package_linux-zen-goatd-gaming-headers() {"),
