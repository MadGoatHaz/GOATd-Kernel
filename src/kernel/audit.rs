...
impl SystemAudit {
    ...
    /// ensuring the operation doesn't block the Slint UI thread.
    ///
    ...
    pub async fn run_deep_audit_async() -> Result<KernelAuditData, String> {
        let audit_data = tokio::task::spawn_blocking(|| {
            collect_kernel_audit_for_version(None)
        })
        .await
    ...
    /// - `Err(String)` if the audit is cancelled or task fails
    pub async fn run_deep_audit_async_for_version(version: Option<String>) -> Result<KernelAuditData, String> {
    ...
    pub async fn run_jitter_audit_async() -> Result<JitterAuditResult, String> {
        let jitter_data = tokio::task::spawn_blocking(|| {
            measure_jitter()
        })
        .await
    ...
    match std::fs::read(vmlinuz_path) {
        Ok(binary) => {
            ...
            if decoder.read_to_string(&mut decompressed).is_ok() && !decompressed.is_empty() {
                log_info!("[KernelAudit] Successfully extracted ikconfig from {}", vmlinuz_path.display());
            ...
            if let Ok(raw_str) = String::from_utf8(compressed_data.to_vec()) {
                if raw_str.contains("CONFIG_") {
                    log_info!("[KernelAudit] Successfully extracted raw ikconfig from {}", vmlinuz_path.display());
                    return Some(raw_str);
                }
            }
            
            log_info!("[KernelAudit] Failed to extract ikconfig from {}", vmlinuz_path.display());
            None
        }
        Err(e) => {
            log_info!("[KernelAudit] Failed to read kernel binary {}: {}", vmlinuz_path.display(), e);
            None
        }
    }
}
...
fn read_kernel_config(version: Option<&str>) -> String {
    ...
    if let Ok(config_str) = std::fs::read_to_string(&boot_config_path) {
        log_info!("[KernelAudit] Successfully read kernel config from {}", boot_config_path);
        return config_str;
    }
    log_info!("[KernelAudit] Failed to read config from {}", boot_config_path);
    ...
    if let Ok(config_str) = std::fs::read_to_string(&lib_config_path) {
        log_info!("[KernelAudit] Successfully read kernel config from {}", lib_config_path);
        return config_str;
    }
    log_info!("[KernelAudit] Failed to read config from {}", lib_config_path);
    ...
    log_info!("[KernelAudit] Could not read kernel config from any source for version {}", kernel_version);
    String::new()
}
...
fn detect_io_scheduler() -> String {
    ...
    log_info!("[KernelAudit] Root filesystem device (raw): {}", if root_device.is_empty() { "unknown" } else { &root_device });
    
    // Extract base block device from the root device string
    let base_device = extract_base_block_device(&root_device);
    log_info!("[KernelAudit] Base block device extracted: {}", if base_device.is_empty() { "unknown" } else { &base_device });
    ...
    match std::fs::read_dir("/sys/block") {
        Ok(entries) => {
                ...
                if let Ok(metadata) = entry.metadata() {
                    if metadata.is_dir() {
                        ...
                        if is_target_device || is_storage {
                            ...
                            log_info!("[KernelAudit] [DEBUG] Checking scheduler path: {}", scheduler_path.display());
                            ...
                            if let Ok(content) = std::fs::read_to_string(&scheduler_path) {
                                ...
                                for scheduler in content.split_whitespace() {
                                    if scheduler.starts_with('[') && scheduler.ends_with(']') {
                                        let scheduler_name = scheduler.trim_matches('[').trim_matches(']').to_string();
                                        ...
                                        let final_scheduler = if scheduler_name == "none" && device_str.starts_with("nvme") {
                                            "Hardware Queues (NVMe)".to_string()
                                        } else {
                                            scheduler_name
                                        };
                                        
                                        found_schedulers.push((device_str.to_string(), final_scheduler, is_target_device));
                                        found_active = true;
                                        break;
                                    }
                                }
            ...
            found_schedulers.sort_by_key(|(device, _, is_target)| {
                if *is_target { 0 } else if device.starts_with("nvme") { 1 } else { 2 }
            });
            
            if let Some((device, scheduler, _)) = found_schedulers.first() {
                log_info!("[KernelAudit] I/O scheduler detected: '{}' (on {})", scheduler, device);
...
fn detect_scx_from_sysfs() -> Option<String> {
    ...
    if state != "enabled" {
        log_info!("[KernelAudit] SCX is not enabled in kernel (state: {})", state);
        return None;
    }
    ...
    match ops_result {
        Ok(content) => {
            ...
            log_info!("[KernelAudit] Active SCX scheduler from sysfs: {}", scheduler_name);
            Some(scheduler_name)
        }
        Err(e) => {
            log_info!("[KernelAudit] Failed to read SCX scheduler from sysfs: {}", e);
            None
        }
    }
}
...
fn detect_cpu_scheduler() -> String {
    ...
    if let Some(scx_name) = detect_scx_from_sysfs() {
        log_info!("[KernelAudit] SCX scheduler detected at kernel layer: {}", scx_name);
    ...
    log_info!("[KernelAudit] [DEBUG] CONFIG_SCHED_BORE detected: {}", has_bore);
    
    let has_sched_core = config_str.contains("CONFIG_SCHED_CORE=y");
    let has_eevdf = config_str.contains("CONFIG_SCHED_EEVDF=y");
    log_info!("[KernelAudit] [DEBUG] CONFIG_SCHED_CORE detected: {}", has_sched_core);
    log_info!("[KernelAudit] [DEBUG] CONFIG_SCHED_EEVDF detected: {}", has_eevdf);
    
    let has_cfs = config_str.contains("CONFIG_SCHED_CFS=y");
    log_info!("[KernelAudit] [DEBUG] CONFIG_SCHED_CFS detected: {}", has_cfs);
    ...
    log_info!("[KernelAudit] [DEBUG] kernel.sched_bore sysctl: {}", if bore_enabled { "enabled" } else { "disabled or not found" });
...
fn count_kernel_modules(kernel_version: Option<&str>) -> (u32, u32) {
    ...
    match std::fs::read_to_string(&modules_order_path) {
        Ok(content) => {
            let order_count = content.lines()
                .filter(|line| !line.trim().is_empty())
                .count() as u32;
            log_info!("[KernelAudit] Standard modules (modules.order): {}", order_count);
            count += order_count;
        }
        Err(e) => {
            log_info!("[KernelAudit] modules.order not found at {}: {}", modules_order_path, e);
        }
    }
        ...
        for entry in dkms_root.flatten() {
            if let Ok(metadata) = entry.metadata() {
                if metadata.is_file() {
                    ...
                    if file_str.ends_with(".ko") || file_str.ends_with(".ko.gz") ||
                       file_str.ends_with(".ko.xz") || file_str.ends_with(".ko.zst") {
                        dkms_count += 1;
                    }
    ...
    log_info!("[KernelAudit] Total module count (modules.order + DKMS): {}", count);
...
fn detect_cpu_family(model_name: &str) -> String {
    if model_name.contains("Ryzen") {
        if model_name.contains("Zen 5") || model_name.contains("9755") || model_name.contains("7755") {
            "ZEN5".to_string()
        } else if model_name.contains("Zen 4") || model_name.contains("9654") || model_name.contains("9004") {
            "ZEN4".to_string()
        ...
        if model_name.contains("Meteor") {
            ...
            "KABY_LAKE".to_string()
        } else if model_name.contains("Coffee") || model_name.contains("8th Gen") || model_name.contains("9th Gen") {
                    ...
                    if dir_str.starts_with("thermal_zone") {
                        ...
                        if let Ok(temp_str) = std::fs::read_to_string(&temp_path) {
                            if let Ok(temp_raw) = temp_str.trim().parse::<f32>() {
                                ...
                                if let Ok(type_str) = std::fs::read_to_string(entry.path().join("type")) {
                                    let zone_type = type_str.trim().to_lowercase();
                                    
                                    // Prefer package/die temperature over core temps
                                    if zone_type.contains("package") || zone_type.contains("x86_pkg_temp") {
            ...
            if let Ok(metadata) = entry.metadata() {
                if metadata.is_dir() {
                    ...
                    if let Ok(temp_str) = std::fs::read_to_string(&pkg_temp_path) {
                        if let Ok(temp_raw) = temp_str.trim().parse::<f32>() {
                            let temp_c = temp_raw / 1000.0;
                            log_info!("[PerformanceAudit] Read package temperature from hwmon: {:.1}°C", temp_c);
                            return Some(temp_c);
                        }
                    }
                }
...
fn detect_mglru_status() -> String {
    ...
    match std::fs::read_to_string("/sys/kernel/mm/lru_gen/enabled") {
        Ok(content) => {
            ...
            log_info!("[KernelAudit] MGLRU sysfs read successful, raw value: '{}'", raw_value);
            
            // Harden parsing: trim, lowercase, remove "0x" prefix, parse as hex
            let cleaned = raw_value.to_lowercase().replace("0x", "");
            log_info!("[KernelAudit] Cleaned MGLRU value for parsing: '{}'", cleaned);
            
            match u32::from_str_radix(&cleaned, 16) {
                Ok(value) => {
                    log_info!("[KernelAudit] MGLRU successfully parsed as hex integer: {}", value);
...
fn collect_kernel_audit_for_version(kernel_version: Option<&str>) -> KernelAuditData {
    ...
    let is_booted = kernel_version.is_none() || kernel_version.map_or(false, |v| v == booted_version);
    ...
    if is_booted {
        ...
        let mglru_status = if config_str.is_empty() {
            ...
            if config_str.contains("CONFIG_LRU_GEN=y") {
                ...
                log_info!("[KernelAudit] CONFIG_LRU_GEN=m found in offline kernel config (module form)");
                "Module (Not loaded)".to_string()
            } else {
...
fn measure_jitter() -> JitterAuditResult {
     ...
     log_info!("[KernelAudit] Jitter measurement: min={:.2}µs, max={:.2}µs, avg={:.2}µs",
...
mod tests {
    ...
    fn test_mglru_parser_0x0007_full() {
        ...
        assert_eq!(result, "Full (Tier+Ref+Thr)", "0x0007 should parse as Full MGLRU");
    }

    #[test]
    fn test_mglru_parser_0x0007_uppercase() {
        let result = parse_mglru_value("0X0007");
        assert_eq!(result, "Full (Tier+Ref+Thr)", "0X0007 (uppercase X) should parse as Full MGLRU");
    }

    #[test]
    fn test_mglru_parser_decimal_7() {
        let result = parse_mglru_value("7");
        assert_eq!(result, "Full (Tier+Ref+Thr)", "Decimal 7 should parse as Full MGLRU");
    }
    ...
    fn test_mglru_parser_unknown_value() {
        ...
        assert!(result.contains("15"), "Unknown should contain the decimal value");
    }

    #[test]
    fn test_mglru_parser_invalid_hex() {
        let result = parse_mglru_value("0xGGGG");
        assert!(result.contains("Unknown"), "0xGGGG should parse as Unknown (invalid hex)");
    }

    #[test]
    fn test_mglru_parser_whitespace_handling() {
        let result = parse_mglru_value("0x0007\n");
        assert_eq!(result, "Full (Tier+Ref+Thr)", "Should handle trailing whitespace");
    }

    #[test]
    fn test_mglru_parser_mixed_case() {
        let result = parse_mglru_value("0x000f");
        assert!(result.contains("Unknown"), "Mixed case hex 0x000f should parse as Unknown");
    }
    ...
    fn test_mglru_all_standard_values() {
        ...
        for (input, expected_substring) in test_cases {
            ...
            assert!(result.contains(expected_substring),
                   "Input '{}' should contain '{}'", input, expected_substring);
        }
    }
}
