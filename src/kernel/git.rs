...
use crate::kernel::pkgbuild::{extract_pkgrel, extract_pkgver};
...
pub fn validate_source_version(path: &Path, expected_version: &str) -> GitResult<bool> {
    ...
    let content = std::fs::read_to_string(&pkgbuild_path).map_err(|e| GitError::Io(e))?;

    // Extract pkgver and pkgrel using reused functions from pkgbuild module
    let pkgver = extract_pkgver(&content).map_err(|e| {
        GitError::Repository(format!("Failed to extract pkgver from PKGBUILD: {}", e))
    })?;

    let pkgrel = extract_pkgrel(&content).map_err(|e| {
        GitError::Repository(format!("Failed to extract pkgrel from PKGBUILD: {}", e))
    })?;
...
impl GitManager {
    ...
    pub fn fetch(&self) -> GitResult<()> {
        ...
        remote
            ...
            .map_err(|e| GitError::Repository(format!("Fetch failed: {}", e)))?;
    ...
    pub fn list_branches(&self) -> GitResult<Vec<String>> {
        ...
        let branches = repo
            .branches(None)
            .map_err(|e| GitError::Repository(format!("Failed to list branches: {}", e)))?;

        let mut branch_names = Vec::new();
        for branch_result in branches {
            let (branch, _) = branch_result
                .map_err(|e| GitError::Repository(format!("Failed to read branch: {}", e)))?;
    ...
    pub fn checkout(&self, reference: &str) -> GitResult<()> {
        ...
        match obj.as_tag() {
            Some(tag) => {
                let target = tag.target().map_err(|e| {
                    GitError::InvalidRef(format!("Failed to get tag target: {}", e))
                })?;
...
mod tests {
    ...
    fn test_git_error_display() {
        ...
        assert_eq!(err.to_string(), "Reference not found: main");
    }
}
